...
/*
Directly from https://github.com/danielbohannon/Revoke-Obfuscation/blob/master/Requirements/RevokeObfuscationHelpers.cs
...
{
    public static OrderedDictionary GetRvoFeatureVector(string input, string label)
    {
        // Function that calls all of the checks implemented by Revoke-Obfuscation on
        ...
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = GroupedArrayElementRangeCounts.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = GroupedAssignmentStatements.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = GroupedAstTypes.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = GroupedBinaryExpressionOperators.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = CmdletMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = CommandParameterNameMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = CommentMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = ConvertExpressionMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = FunctionNameMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = IntegerAndDoubleMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = InvocationOperatorInvokedObjectMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = LineByLineMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = MemberArgumentMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = MemberMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = StringMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = TypeConstraintMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = TypeExpressionMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = GroupedUnaryExpressionOperators.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        result = VariableNameMetrics.AnalyzeAst(ast);
        foreach (DictionaryEntry entry in result)
        {
            allCheckScriptResults.Add((string)entry.Key, (double)entry.Value);
        }

        allCheckScriptResults.Add("Label", label);
